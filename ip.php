...
<center><h4>IP meklēšana</h4></center>
								<form action="" method="get" class="pull-right col-xs-12 col-sm-6">
							<div class="input-group no-margin">
								<span class="input-group-addon" style="border:none;background: #fff;background: rgba(0,0,0,.05);"><i class="fa fa-search"></i></span>
								<input type="text" name="lol" placeholder="Meklēt..." class="form-control no-padding-hr" style="border:none;background: #fff;background: rgba(0,0,0,.05);">
							</div>
						</form>
						<?php
if(isset($_GET['lol']))
{
	$ip = $_GET['lol'];
	$check = "87.110.247.136";
	if($ip == $check)
	{
		?>
		<br>
		<br><p><div class="alert alert-page alert-danger alert-dark" style="width:950px; margin-left:auto; margin-right:auto;">
						<strong>UPS!</strong> Šo IP ir aizliegts uzmeklēt.
					</div></p>
		<?php
	}
	else
	{
		$okey = "1";
        echo "<p><table class='table table-bordered' align='center' width='750' cellspacing='0' cellpadding='0'>
        <tr>
        <td width='200' align='center'><strong>Spēlētājs</strong></td>
	    <td width='540' align='center'><strong>Pēdējo reizi bijis ar šo IP</strong></td>
          </tr>";		
	}
}
?>
...
if(isset($_GET['lol'])) 
{
	if($okey == 1)
	{
	    $orgq = "SELECT name,ip FROM accounts WHERE ip = '$ip'";
	    $orgres = mysqli_query($connect,$orgq);
	    while ($bans = mysqli_fetch_array($orgres))
	    {
	    do
	    {
		    echo "<tr align='center'><td>".$bans['name']."</td><td>".$bans['ip']."</td>";
	    }
        while ($bans = mysqli_fetch_array($orgres));
        }	
	}
...
?>
						...
						<strong>UPS!</strong> Tev uz šo sadaļu nav pieejas, uz to ir pieeja tikai sākot no 3 līmeņa administrātora.
					</div></p>
								<?php
