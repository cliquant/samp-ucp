...
{ 
	...
	$tq = "SELECT time FROM paintball LIMIT 1";  
	$tres=mysqli_query($connect,$tq);
    while($time = mysqli_fetch_assoc($tres)) 
    { 
		$ptime = $time['time'];
	}
} 
...
<center><nav class="navbar navbar-default"  style="width:1000px;margin-left:0px;margin-top:2px;">
 ...
 <div class="lol" style="width:1000px; position:relative; margin-right:auto; margin-left:auto;">
 <div class="panel-body" style="width:1000px; margin-left:auto; margin-right:auto;">
<?php 
$orgq = "SELECT model FROM paintball ORDER by pkill DESC LIMIT 3";
$orgres = mysqli_query($connect,$orgq);
$orgcount = mysqli_num_rows($orgres);
if($orgcount == 3){
	echo '<center><div class="alert alert-page alert-success alert-dark" style="width:1000px; margin-top:-5px;">
						<strong>'.date("d.m.y",$ptime).'</strong> Uzvarētāji iepriekšējā nedēļā.
					</div></center>';
    while ($bans = mysqli_fetch_array($orgres))
    {
        do
        {
			...
			$a++;
			if($a == 1){
	            echo "<div class='col-sm-3' style='width:320px;'>
					<div class='panel-heading'>
						<center><span class='panel-title'>Pirmā vieta</span></center>
					</div>
					<center><div class='plan-pricing bg-light-white'><img style='width:200px; height:250px;' src = 'skins/".$skin.".png' /></div></center>
                    </div>
			    ";
			}
			if($a == 2){
	            echo "<div class='col-sm-3' style='width:320px;'>
					<div class='panel-heading'>
						<center><span class='panel-title'>Otrā vieta</span></center>
					</div>
					<center><div class='plan-pricing bg-light-white'><img style='width:200px; height:250px;' src = 'skins/".$skin.".png' /></div></center>
                    </div>
			    ";
			...
			if($a == 3){
	            echo "<div class='col-sm-3' style='width:320px;'>
					<div class='panel-heading'>
						<center><span class='panel-title'>Trešā vieta</span></center>
					</div>
					<center><div class='plan-pricing bg-light-white'><img style='width:200px; height:250px;' src = 'skins/".$skin.".png' /></div></center>
                    </div>
			    ";
...
<table class="table table-bordered">
						...
						<tbody>
							<?php 
$pq = "SELECT nick,pkill,pdead FROM paintball ORDER by pkill DESC LIMIT 3";
$pres = mysqli_query($connect,$pq);
$RCoins = 15;
while ($paint = mysqli_fetch_array($pres))
{
do
{
	$b++;
	$KD = $paint['pkill'] / $paint['pdead'];
	$res = substr($KD, 0, 4);
    echo "<tr class='gradeA'><td>".$b."</td><td>".$paint['nick']."</td><td>".$paint['pkill']."</td><td>".$paint['pdead']."</td><td>".$res."</td><td>".$RCoins." RCoins</td></tr>";
	$RCoins -= 5;
}
while ($bans = mysqli_fetch_array($orgres));
}
	...
	?>
	<center><div class="alert alert-page alert-danger alert-dark" style="width:1000px; margin-top:-5px;">
						<strong>Uzmanību!</strong> Diemžēl sistēma iepriekšējā nedēļā nesavāca vismaz 3 labākos!
					</div></center>
	<?php
}
?>
